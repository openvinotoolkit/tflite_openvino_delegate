...
#include "delegate/intel_openvino/operations/include/mean.h"
...
class OpenVINOGraphBuilder {
 public:
  ...
  TfLiteStatus convertNHWCtoNCHW(std::vector<int> node_dims,
    ...
    if (node_dims.size() < 4 && node_dims.size() > 0) {
      ...
      auto new_size = std::make_shared<ov::opset8::Constant>(
          ov::element::i32, ov::Shape{4}, node_dims);
    ...
    transposed_node =
        std::make_shared<ov::opset3::Transpose>(input, order_node);
  ...
  TfLiteStatus AddInputParams(const TfLiteOpaqueTensor *t, const int index) {
    ...
    if (convertNHWCtoNCHW(dims, input, interim) != kTfLiteOk)
      return kTfLiteError;
  ...
  TfLiteStatus CreateConstNode(const TfLiteOpaqueContext *context,
    ...
    switch (tensor_type) {
      ...
      case kTfLiteInt4:
        ...
        break;
      default:
        TFLITE_LOG(ERROR) << "Element type " << tensor_type
                          << " not supported\n";
        return kTfLiteError;
    }
  ...
  TfLiteStatus UpdateResultNodes(const TfLiteOpaqueContext *context,
    ...
    for (auto o : outputs) {
      ...
      if (dims.size() == 4) {
        ...
        out_node =
            std::make_shared<ov::opset3::Transpose>(out_node, order_node);
        if (out_node == NULL) {
  ...
  TfLiteStatus CreateOpClass(int operationIndex,
                             TfLiteRegistrationExternal *registration,
